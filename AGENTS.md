...
- **Run `npx prisma generate` before `npm run build`/tests.** `src/common/guards/roles.guard.ts` imports `Role` from `@prisma/client`; Prisma 7 emits the client to `node_modules/@prisma/client` at generate time. Type-aware ESLint needs it too — `npm run lint` has a `prelint` step that runs `prisma generate` first; if you run `eslint` directly with a stale/missing client, every `this.prisma.*` call reports `@typescript-eslint/no-unsafe-call`/`no-unsafe-assignment` ("could not be resolved").
